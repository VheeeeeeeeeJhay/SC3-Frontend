    ...
    <div class="bg-white/10 dark:bg-black/30 backdrop-blur-lg rounded-3xl shadow-2xl w-full max-w-5xl p-10 transition-all duration-300 border border-white/10 dark:border-gray-800">
      <div class="flex flex-col md:flex-row items-center justify-between">
        ...
        <div class="md:w-1/3 text-center mb-10 md:mb-0">
          <div class="w-48 h-48 mx-auto mb-6 flex items-center justify-center rounded-full bg-gradient-to-br from-teal-600 to-emerald-700 text-white text-7xl font-bold shadow-2xl border-4 border-white/20 dark:border-gray-800">
          ...
          <button
            class="mt-6 bg-teal-600 hover:bg-teal-700 text-white px-6 py-2.5 rounded-full shadow-md transition-transform hover:scale-105"
        ...
        <div class="md:w-2/3 md:pl-12">
          ...
          <ul class="space-y-4 text-base text-gray-700 dark:text-gray-200">
            <li><strong class="text-teal-400 dark:text-teal-300">First Name:</strong> {{ user?.firstName }}</li>
            <li><strong class="text-teal-400 dark:text-teal-300">Middle Name:</strong> {{ user?.middleName ||''}}</li>
            <li><strong class="text-teal-400 dark:text-teal-300">Last Name:</strong> {{ user?.lastName }}</li>
            <li><strong class="text-teal-400 dark:text-teal-300">Email:</strong> {{ user?.email }}</li>
          </ul>
        </div>
      </div>
    </div>
    ...
    <transition name="fade">
      <div v-if="showModal" class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center">
        <div class="bg-white dark:bg-gray-900 border border-white/10 dark:border-gray-700 rounded-3xl shadow-2xl w-full max-w-2xl p-8 relative animate-glow">
          <h3 class="text-2xl font-bold text-teal-600 dark:text-white mb-8 text-center">Edit Profile</h3>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" v-model="editData.firstName"
                class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white/90 dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500" />
            </div>
            <div>
          ...
          <div class="flex justify-between items-center mt-10">
            ...
            <div class="flex gap-3">
              ...
              <button
                @click="saveChanges"
                class="px-5 py-2.5 rounded-xl bg-teal-600 text-white hover:bg-teal-700 transition font-semibold">
                Save
              </button>
            </div>
          </div>
    ...
    <transition name="fade">
      <div v-if="showPasswordModal" class="fixed inset-0 z-60 bg-black/70 backdrop-blur-sm flex items-center justify-center">
        <div class="bg-white dark:bg-gray-900 border border-white/10 dark:border-gray-700 rounded-3xl shadow-2xl w-full max-w-xl p-8 animate-glow">
          <h4 class="text-xl font-semibold text-teal-600 dark:text-teal-400 mb-6 text-center">Change Password</h4>
          <div class="space-y-4">
            <div>
